...
// The returned ErrorData must be checked for errors and destroyed with DestroyErrorData.
func NewDataChunkFromArrow(conn Connection, rec arrow.RecordBatch, schema ArrowConvertedSchema) (DataChunk, ErrorData) {
	var cd C.duckdb_data_chunk
	chunk := DataChunk{Ptr: unsafe.Pointer(cd)}

	return chunk, MoveArrowToDataChunk(conn, rec, schema, chunk)
}
...
// The returned DataChunk must be destroyed with DestroyDataChunk.
// The returned ErrorData must be checked for errors and destroyed with DestroyErrorData.
func MoveArrowToDataChunk(conn Connection, rec arrow.RecordBatch, schema ArrowConvertedSchema, chunk DataChunk) ErrorData {
	arr := C.calloc(1, C.sizeof_struct_ArrowArray)
	defer func() {
		cdata.ReleaseCArrowArray((*cdata.CArrowArray)(arr))
		C.free(arr)
